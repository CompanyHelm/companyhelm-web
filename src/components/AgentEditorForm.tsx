...
export function AgentEditorForm({
  ...
  const editingRunnerCodexModelEntries = getRunnerCodexModelEntriesForRunner(
    runnerCodexModelEntriesById,
  ...
  const editingRunnerModelNames = getRunnerModelNames(editingRunnerCodexModelEntries);
  const editingRunnerReasoningLevels = getRunnerReasoningLevels(editingRunnerCodexModelEntries, editingDraft.model);
  ...
  useEffect(() => {
    ...
    return () => setIsInstructionsFullscreen(false);
  }, [isInstructionsFullscreen]);
  ...
  return (
    <form className="task-form" onSubmit={handleSubmit}>
      <div className="agent-edit-grid">
        <label className="relationship-field" htmlFor={`edit-agent-runner-${agent.id}`}>
          Runner
        </label>
        <select
        ...
        <label className="relationship-field" htmlFor={`edit-agent-name-${agent.id}`}>
          Name
        </label>
        <input
        ...
        <label className="relationship-field" htmlFor={`edit-agent-sdk-${agent.id}`}>
          SDK
        </label>
        <select
        ...
        <label className="relationship-field" htmlFor={`edit-agent-model-${agent.id}`}>
          Model
        </label>
        <select
        ...
        <label className="relationship-field" htmlFor={`edit-agent-reasoning-${agent.id}`}>
          Reasoning
        </label>
        <select
        ...
        <label
          ...
          htmlFor={`edit-agent-default-additional-model-instructions-${agent.id}`}
        >
        ...
        <div className="edit-agent-instructions-field">
          ...
          <textarea
            id={`edit-agent-default-additional-model-instructions-${agent.id}`}
        ...
        <label className="relationship-field" htmlFor={`edit-agent-roles-${agent.id}`}>
          Assigned roles
        </label>
        <div id={`edit-agent-roles-${agent.id}`} className="inline-selection-list">
          {editingRoleIds.length === 0 ? (
            ...
            editingRoleIds.map((roleId) => {
              ...
              return (
                <button
                  key={`edit-agent-role-${agent.id}-${roleId}`}
                  type="button"
                  ...
                  onClick={() =>
                    onAgentDraftChange(
                      agent.id,
                      "roleIds",
                      editingRoleIds.filter((candidateId) => candidateId !== roleId),
                    )
                  }
                  disabled={isEditingDisabled}
        ...
        <label className="relationship-field" htmlFor={`edit-agent-role-add-${agent.id}`}>
          Add role
        </label>
        <select
          id={`edit-agent-role-add-${agent.id}`}
          value=""
          onChange={(event: ChangeEvent<HTMLSelectElement>) => {
        ...
        >
          <option value="">
            {editingAvailableRoles.length === 0 ? "All roles already assigned" : "Select role to assign"}
          </option>
        ...
        <label className="relationship-field" htmlFor={`edit-agent-effective-skills-${agent.id}`}>
          Inherited skills
        </label>
        <div id={`edit-agent-effective-skills-${agent.id}`} className="inline-selection-list">
          {editingEffectiveSkills.length === 0 ? (
            ...
            editingEffectiveSkills.map((skill) => (
              <span key={`edit-agent-effective-skill-${agent.id}-${skill.id}`} className="tag-pill">
                {skill.name}
              </span>
            ))
          )}
        ...
        <label className="relationship-field" htmlFor={`edit-agent-direct-skills-${agent.id}`}>
          Direct skills
        </label>
        <div id={`edit-agent-direct-skills-${agent.id}`} className="inline-selection-list">
          {editingSkillIds.length === 0 ? (
            ...
            editingSkillIds.map((skillId) => {
              ...
              return (
                <button
                  key={`edit-agent-direct-skill-${agent.id}-${skillId}`}
                  type="button"
                  className="tag-remove-btn"
                  onClick={() =>
                    onAgentDraftChange(
        ...
        <label className="relationship-field" htmlFor={`edit-agent-direct-skill-add-${agent.id}`}>
          Add direct skill
        </label>
        <select
          id={`edit-agent-direct-skill-add-${agent.id}`}
          value=""
          onChange={(event: ChangeEvent<HTMLSelectElement>) => {
        ...
        >
          <option value="">
            {editingAvailableSkills.length === 0 ? "All skills already assigned" : "Select skill to assign"}
          </option>
        ...
        <label className="relationship-field" htmlFor={`edit-agent-effective-mcp-${agent.id}`}>
          Inherited MCP servers
        </label>
        <div id={`edit-agent-effective-mcp-${agent.id}`} className="inline-selection-list">
          {editingEffectiveMcpServerIds.length === 0 ? (
            ...
            editingEffectiveMcpServerIds.map((mcpServerId) => {
              ...
              return (
                <span key={`edit-agent-effective-mcp-${agent.id}-${mcpServerId}`} className="tag-pill">
        ...
        <label className="relationship-field" htmlFor={`edit-agent-direct-mcp-${agent.id}`}>
          Direct MCP servers
        </label>
        <div id={`edit-agent-direct-mcp-${agent.id}`} className="inline-selection-list">
          {editingMcpServerIds.length === 0 ? (
            ...
            editingMcpServerIds.map((mcpServerId) => {
              ...
              return (
                <button
                  key={`edit-agent-direct-mcp-${agent.id}-${mcpServerId}`}
                  type="button"
                  className="tag-remove-btn"
                  onClick={() =>
                    onAgentDraftChange(
        ...
        <label className="relationship-field" htmlFor={`edit-agent-direct-mcp-add-${agent.id}`}>
          Add direct MCP server
        </label>
        <select
          id={`edit-agent-direct-mcp-add-${agent.id}`}
          value=""
          onChange={(event: ChangeEvent<HTMLSelectElement>) => {
        ...
        >
          <option value="">
            {editingAvailableMcpServers.length === 0 ? "All MCP servers already assigned" : "Select MCP server to assign"}
          </option>
