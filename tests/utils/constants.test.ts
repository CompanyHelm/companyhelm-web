...
test("visible navigation uses the reorganized product-domain sections", () => {
  ...
  assert.deepEqual(visibleSections, [
    {
      label: "Workspace",
      itemIds: ["dashboard", "tasks", "my-tasks", "org", "conversations", "chats", "questions"],
      itemHrefs: ["/dashboard", "/tasks", "/my-tasks", "/actors", "/conversations", "/chats", "/questions"],
    },
    {
      label: "AI Studio",
  ...
  assert.equal(PAGE_IDS.has("questions"), true);
  assert.equal(PAGE_IDS.has("my-tasks"), true);
